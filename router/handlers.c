...
*/
void handle_index(const ParsedRequest *req, char *out, int out_size) {
...
*/
void handle_echo(const ParsedRequest *req, char *out, int out_size) {
    char body[1024];
    if (req->query_string[0]) {
        //decoded chars and will format and output
        char decoded[1024];
        url_decode(req->query_string, decoded, sizeof(decoded));
...
*/
void handle_headers(const ParsedRequest *req, char *out, int out_size) {
    char body[4096];
    int  pos = 0;
...
*/
void handle_health(const ParsedRequest *req, char *out, int out_size) {
...
/*
    ...
    as it was received
*/
void handle_post_echo(const ParsedRequest *req, char *out, int out_size) {
    if (req->body_len > 0) {
 ...
 */
void handle_not_found(const ParsedRequest *req, char *out, int out_size) {
 ...
 */
void handle_method_not_allowed(const ParsedRequest *req, char *out, int out_size) {
 ...
 */
void handle_bad_request(const ParsedRequest *req, char *out, int out_size) {
    (void)req;
 ...
 */
void handle_internal_error(const ParsedRequest *req, char *out, int out_size) {
    (void)req;
